...
(function (window, $) {
    ...
    $("body").on("keydown.fv", ".filter-currency", function (e) {
        ...
        if($(this).val().length > 0) {
            if(!validAmount.test(candidateCurrency)) {
    ...
    $("body").on("keydown.fv", ".filter-credit-card", function (e) {
        ...
        if($(this).val().length > 0) {
            if(!validCard.test(candidateCardNumber) || !Validate(candidateCardNumber)) {
                $(this).removeClass("input-valid").addClass("input-invalid");
            } else {
                $(this).removeClass("input-invalid").addClass("input-valid");
            }
    ...
    function ShowErrorSummary(element, beforeOrAfter, heading, errors, colorInputs) {
        ...
        if (theElement !== null) {
            ...
            errorString += "</ul></div>";
            if(beforeOrAfter.toUpperCase() === "BEFORE") {
                theElement.before(errorString);
            } else {
                theElement.after(errorString);
            }
        }
    }
    ...
    function ShowMessage(element, beforeOrAfter, type, header, message, timeout) {
        ...
        if (theElement !== null) {
            ...
            if(beforeOrAfter.toUpperCase() === "BEFORE") {
                theElement.before(messageString);
            } else {
                theElement.after(messageString);
            }
            if (timeout !== null) {
                //setTimeout
                setTimeout(function () {
    ...
    function Validate(Luhn) {
        ...
        if (Calculate(LuhnLess) === parseInt(LuhnDigit, 10)) {
            return true;
        }
        return false;
    }
